...
-(void) checkerActionOnButton:(NormalButton*)button atIndex:(int)index{
    ...
    if (selectedData[@"Image"] != [NSNull null]){
        ...
        [MODEL.requester startPostRequest:checkerModel completeHandler:^(HTTPRequester*requester, ResponseJsonModel *data, NSHTTPURLResponse *httpURLReqponse, NSError *error) {
            NSLog(@"hero");
        }];
    }else{
...
-(void) didFinishReceiveData:(HTTPRequester *)request data:(ResponseJsonModel *)data{
    ...
    if ([request.identification isEqualToString:@"uploaded"]){
        ...
        if (uploadedPath){
            ...
            [MODEL.requester startPostRequest:updatePathModel completeHandler:^(HTTPRequester*requester, ResponseJsonModel *data, NSHTTPURLResponse *httpURLReqponse, NSError *error) {
                NSLog(@"update path");
            }];
        }
    }
        ...
        [MODEL.requester startPostRequest:orderModel completeHandler:^(HTTPRequester* requester,ResponseJsonModel *data, NSHTTPURLResponse *httpURLReqponse, NSError *error) {
