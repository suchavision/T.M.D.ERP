...
- (void)viewDidLoad
{
    ...
    _titleOld.textFieldDidClickAction = ^void(JRTextField* sender) {
          ...
          PickerModelTableView* pickView = [PickerModelTableView popupWithRequestModel:@"ArticlesOrder" fields:needFields ];
...
-(void)controlModeSetting
{
    if (self.controlMode == JsonControllerModeCreate)
    {
        ...
        NSArray* fetchedObjects = [ArticlesObjectHelper fetchManagedObject:MODEL.signedUserName];
...
-(void) didSuccessSendObjects: (NSMutableDictionary*)objects response:(ResponseJsonModel*)response
{
    ...
    // delete cache
    [ArticlesObjectHelper deleteManagedObject:MODEL.signedUserName];
