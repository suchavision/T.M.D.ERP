...
//    [self backBackHidden:YES];
    ...
    id<DataProvider>store = [Store sharedStore];
    [store initiateWithComplete:^(NSError* error){
        if (!error){
            self.browser = [[self childViewControllers] firstObject];
            self.browser.current_path = [[Store sharedStore] rootPath];
            ...
            NSArray* photos = [[Store sharedStore] photosArray];
...
-(void) WDBrowser:(BrowserViewCotroller *)browser didUpdateDataWithPath:(NSString *)path{
    NSDictionary* newData = [[Store sharedStore] dataWithPath:path];
    [self.browserDataSource updateItems:newData];
    [self.photoDataSource updateItems:[[Store sharedStore] photosArray]];
}
...
-(void) newAlbumController:(NewAlbumController *)controller didDoneWithName:(NSString *)name passwd:(NSString *)passwd {
    ...
    [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    [[Store sharedStore] createAlbumAtPath:self.browser.current_path name:name passwd:passwd complete:^(NSError* error, id current_data){
...
-(void) exportCardController:(ExportCardController *)controller didExportToItem:(id)item{
    ...
    [[Store sharedStore] moveItemWithName:names toPath:((Card*)item).path.path complete:^(NSError *error) {
        if (!error){
            [MBProgressHUD hideHUDForView:self.view animated:YES];
            NSDictionary* newData = [[Store sharedStore] dataWithPath:self.browser.current_path];
            [self.browserDataSource updateItems:newData];
            [self.browser refresh];
        }
    }];
...
- (void)popupTextView:(YIPopupTextView*)textView willDismissWithText:(NSString*)text{
    ...
    if (textView.tag == 12345){
        ...
        [MBProgressHUD showHUDAddedTo:self.view animated:YES];
        __block __weak id<DataProvider> store = [Store sharedStore];
...
- (IBAction)exportAction:(id)sender {
    ...
    NSArray* albums = [[Store sharedStore] getAlbums];
...
- (IBAction)cameraAction:(id)sender {
    if ([self.browser.current_path isEqualToString:[Store sharedStore].rootPath]){
...
- (IBAction)deleteAction:(id)sender {
    ...
    if (selected.count){
        //        [[Store sharedStore] removeItemAtIndexPaths:selected];
        //        [self.browser refresh];
        ...
        [MBProgressHUD showHUDAddedTo:self.view animated:YES];
        [[Store sharedStore] removeItemWithNames:result complete:^(NSError *error, id current_data) {
            if (error){
                NSLog(@"delete failed");
            }else{
                [self.browserDataSource updateItems:current_data];
