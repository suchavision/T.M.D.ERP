...
- (void)viewDidLoad
{
    ...
    editDepartmentTx.textFieldDidClickAction = ^void(JRTextField* jrTextField) {
        ...
        departmentEditTableView.tableView.tableView.tableViewBaseDidSelectIndexPathAction = ^(TableViewBase* tableViewObj, NSIndexPath* indexPath){
    ...
    expTableView.tableViewBaseDidSelectIndexPathAction = ^void(TableViewBase* tableViewObj, NSIndexPath* indexPath){
    ...
    buttonAdd.didClikcButtonAction = ^void(JRButton* buttonAdd) {
        if ([KeyBoardHelper sharedInstance].isKeyboardShowing) [ViewHelper resignFirstResponserOnView: buttonAdd.superview];
    ...
    // Print button
    JRButton* printButton = (JRButton*)[infoNestedView getView:@"NESTED_RIGHT.QR.Print"];
    printButton.didClikcButtonAction = ^void(JRButton* btn) {
        [[PopupViewHelper popSheet:nil inView:[ViewHelper getTopView] actionBlock:^(UIView *popView, NSInteger index) {
            // QR CODE
            if (index == 0) {
                NSMutableDictionary* loadImagePathsRepository = [JsonControllerHelper getImagesPathsInController: self];
                NSString* QRCODE_path = loadImagePathsRepository[@"IMG_QRCode"];
                [VIEW.progress show];
                [AppServerRequester getImage:QRCODE_path completeHandler:^(id identification, UIImage *image, NSError *error) {
                    [VIEW.progress hide];
                    [ViewHelper print:image completeHandler:nil];
                }];
            } else if (index == 1) {
                [ViewHelper printView: jsonView completeHandler:nil];
            }
            
        } buttons:APPLOCALIZE_KEYS(@"Print", @"QRCode"),APPLOCALIZE_KEYS(@"Print",@"Employee"), APPLOCALIZE_KEYS(@"CANCEL"), nil] setCancelButtonIndex: 2];
    };
    ...
    self.didShowTabView = ^void(int index, JsonDivView* tabView) {
        ...
        if ([buttonKey isEqualToString:@"ZZ_TAB_BTNPending"]) {
            ...
            tableView.tableView.tableViewBaseWillShowCellAction = ^void(TableViewBase *tableViewObj, UITableViewCell *cell, NSIndexPath *indexPath) {
                ...
                NSString* department = [MODEL.modelsStructure getCategory: orderType];
            ...
            if (!employeeNumber) {
                employeeNumber = MODEL.signedUserName;
            }
...
-(void) assembleResults: (NSMutableDictionary*)realContentsDictionary tableView:(JRRefreshTableView*)headerTableView
{
    ...
    [TableViewBaseContentHelper iterateContents: realContentsDictionary handler:^BOOL(id key, int section, id cellContent) {
    ...
    headerTableView.tableView.tableViewBaseDidSelectIndexPathAction = ^void(TableViewBase* tableViewObj, NSIndexPath* indexPath) {
