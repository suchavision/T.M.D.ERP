...
-(void) subRender: (NSDictionary*)dictionary {
    ...
    if (dictionary[k_JR_ISNAME_NUMBER]) {
        ...
        self.textFieldDidClickAction = ^void(JRTextField* nameNOTextfield) {
            PickerModelTableView* pickView = [PickerModelTableView popupWithModel:nameNOTextfield.memberType];
...
+(void) setTextFieldBoolValueAction: (JRTextField*)textField keys:(NSArray*)keys titleKey:(NSString*)titleKey
{
    ...
    textField.textFieldDidClickAction = ^void(JRTextField* textField) {
        [PopupTableHelper showPopTableView: textField titleKey:titleKey dataSources: [LocalizeHelper localize: textField.booleanValuesLocalizeKeys]];
    };
}
