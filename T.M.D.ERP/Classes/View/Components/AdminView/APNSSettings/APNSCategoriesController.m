...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (! headerView) {
        ...
        titleLabel.frame = CanvasRect(0, 0, 0, 50);
    ...
    if (indexPath.section == 0)
    {
        ...
        if ([cellText isEqualToString: workTime])
        {
            ...
            controller.viewDidLoadBlock = ^void(BaseController* controllerObj){
                ...
                [FrameHelper translateSubViewsFramesRecursive: jsonDivView handler:nil];
                
                for (UIView* subView in jsonDivView.subviews) {
                    if ([subView isKindOfClass: [JsonDivView class]]) {
                        [LayerHelper setAttributesValues: @{@"borderColor": @(7), @"borderWidth": @(1), @"cornerRadius": @(5), @"masksToBounds": @(TRUE)} layer:subView.layer];
        ...
        if ([cellText isEqualToString: materialRequisition]) {
            ...
            controller.viewDidLoadBlock = ^void(BaseController* controllerObj){
                ...
                productNameTextField.textFieldDidClickAction = ^void(JRTextField* productNameTx) {
                    PickerModelTableView* pickerTableView = [PickerModelTableView popupWithRequestModel: MODEL_WHInventory fields:@[@"productCode", @"productName"]];
                    pickerTableView.tableView.headersXcoordinates = @[@(100),@(300)];
                    pickerTableView.tableView.tableView.tableViewBaseDidSelectIndexPathAction = ^void(TableViewBase* tableViewObj, NSIndexPath* indexPath){
