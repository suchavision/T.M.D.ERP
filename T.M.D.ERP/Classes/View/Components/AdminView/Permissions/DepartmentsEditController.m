...
- (void)viewDidLoad
{
    categories = [[MODEL.usersNOPermissions objectForKey: userNumber] objectForKey:CATEGORIES];
    permissions = [[MODEL.usersNOPermissions objectForKey: userNumber] objectForKey:PERMISSIONS];
    [super viewDidLoad];
    ...
    self.wheelDidTapSwipLeftBlock = ^(AppWheelViewController* wheel, NSInteger index){
        ...
        NSArray* departmentOrders = [MODEL.modelsStructure getOrders: department withBill:NO];
...
+(void) saveUserPermissions: (NSString*)userNumber categories:(NSMutableArray*)categories permissions:(NSMutableDictionary*)permissions completion:(void(^)(NSError* error))completion;
{
    ...
    [EVENT.adminAction savePermissions: userNumber permissions:compressPermissionJson categories:compressCategoryJson completeHandler:^(NSError *errorObj) {
        if (errorObj) {
