...
-(void)setModelName:(NSString *)modelName
{
    ...
    approvals = [MODEL.modelsStructure getModelApprovals: modelName];
}
...
//        [ColorHelper setBorder: panelView];
        ...
        approvalView.addButton.didClikcButtonAction = ^void(NormalButton* button) {
            ...
            for (NSString* username in [MODEL.usersNOApproval allKeys]){
                if ([MODEL.usersNOApproval[username] boolValue]) {
            ...
            pickView.titleHeaderViewDidSelectAction = ^void(JRTitleHeaderTableView* headerTableView, NSIndexPath* indexPath){
                ...
                [PopupViewHelper popAlert: nil message:LOCALIZE_MESSAGE_FORMAT(@"AskSureToAddApproval", MODEL.usersNONames[userNumber]) style:0 actionBlock:^(UIView *popView, NSInteger index) {
                    if (index == 1) {
                        [TableViewBaseContentHelper insertToFirstRowWithAnimation: weakApprovalView.tableView section:0 content:contents realContent:userNumber];
                    }
        ...
        // set frame
        approvalView.frame = CanvasRect(0, baselineY, 500, 400);
        ...
        levelView.frame = CanvasRect(0, 420, 500, 60);
        ...
        levelTextField.frame = CanvasRect(150, 15, 100, 30);
...
{
    NSDictionary* orderAppSettings = [[MODEL.approvalSettings objectForKey: self.categoryName] objectForKey: self.modelName];
    for (int i = 0; i < approvals.count; i++) {
        ...
        approvalView.tableView.tableViewBaseWillShowCellAction = ^void(TableViewBase* tableViewObj,UITableViewCell* cell, NSIndexPath* indexPath) {
            ...
            NSString* tip = nil;
            if (! MODEL.usersNONames[username]) {
