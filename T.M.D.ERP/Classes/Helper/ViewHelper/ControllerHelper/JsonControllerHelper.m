...
+(NSMutableDictionary*) assembleJsonViewSpecifications: (NSString*)order
{
    ...
    [self merge: shareViewSpecifications with:viewSpecifications];
...
+(NSMutableDictionary*) assembleJsonControllerSpecifications: (NSString*)order
{
    ...
    [self merge: shareControllerSpecifications with:controllerSpecifications];
    return shareControllerSpecifications;
}

+(void) merge: (NSMutableDictionary*)destination with:(NSDictionary*)source {
    NSEnumerator* sourceEnumerator = [source keyEnumerator];
    id key;
    while ((key = [sourceEnumerator nextObject])) {
        NSObject* s = source[key];
        NSObject* d = destination[key];
        if ([s isKindOfClass: [NSDictionary class]] && [d isKindOfClass: [NSDictionary class]]) {
            [self merge: (NSMutableDictionary*)d with:(NSDictionary*)s];
        } else {
            [destination setObject: s forKey: key];
        }
    }
}
    ...
    for (NSString* attribute in attributes) {
        ...
        if ([format isEqualToString: json_CHECK_FORMAT_DIGIT]) {
            if ([value isKindOfClass: [NSNumber class]]) {
                ...
                isRightFormat = [StringHelper isNumericValue: value];
            }
        }
        if (! isRightFormat) {
...
+(void) setupAutoFillComponents: (JsonView*)jsonView config:(NSDictionary*)clientConfig
{
    ...
    for (NSString* keyPath in fillUserKeyPath) {
        ...
        [jrview setValue: MODEL.usersNONames[MODEL.signedUserName]];
    }
    // Fill Current Date
...
{
    if (VIEW.navigator.viewControllers.count < 2)
        return;
    ...
    if (isNexPage) {
        [listController getNextRow: 0 handler:^(id identification) {
            if (identification) {
                ...
                [AppViewHelper alertMessage: LOCALIZE_MESSAGE(MESSAGE_TheLastPage)];
            }
        }];
    } else {
        [listController getPreviousRow: 0 handler:^(id identification) {
            if(identification) {
                ...
                [AppViewHelper alertMessage: LOCALIZE_MESSAGE(MESSAGE_TheFirstPage)];
            }
        }];
    }
}
...
+(void) enableExceptionButtonAfterFinalApprovas:(JsonController*)jsonController objects:(NSDictionary*)objects
{
    ...
    NSArray* approvalsKeys = [MODEL.modelsStructure getModelApprovals: jsonController.order];
    
    BOOL isUserInApprovals = [MODEL.signedUserName isEqualToString: objects[PROPERTY_CREATEUSER]];
    BOOL isAllApproved = YES;
    for (NSString* applevel in approvalsKeys) {
        BOOL isApproved = !OBJECT_EMPYT(objects[applevel]);
        
        if (isApproved) {
            if (!isUserInApprovals && [MODEL.signedUserName isEqualToString: objects[applevel]]) {
...
{
    NSArray* levels = [MODEL.modelsStructure getModelApprovals: orderType];
...
{
    NSString* lastLevel = [[MODEL.modelsStructure getModelApprovals: order] lastObject];
    return lastLevel;
}
...
{
    NSArray* approvals = [MODEL.modelsStructure getModelApprovals: orderType];
...
+(void) enableSubmitButtonsForApplyMode: (JsonController*)jsoncontroller withObjects:(NSDictionary*)objects order:(NSString*)order
{
    ...
    [JsonControllerHelper iterateJsonControllerSubmitButtonsConfig: jsoncontroller handler:^BOOL(NSString* buttonKey, JRButton *submitBTN, NSString *departmentType, NSString *orderType, NSString *sendNestedViewKey, NSString *appTo, NSString *appFrom, JsonControllerSubmitButtonType buttonType) {
        ...
        if (isReturned) {
            ...
            BOOL isApplyMode = [self isApplyMode:buttonType objects:objects applevel:appFrom];
...
+(BOOL) isApplyMode: (JsonControllerSubmitButtonType)buttonType objects:(NSDictionary*)objects applevel:(NSString*)applevel
{
    if (buttonType != JsonControllerSubmitButtonTypeApprove) {
        return NO;;
    }
    
    if (![MODEL.signedUserName isEqualToString: objects[PROPERTY_FORWARDUSER]]) {
        return NO;;
    }
...
+(void) renderControllerModeWithSpecification: (JsonController*)jsonController
{
    ...
    NSDictionary* controllModeConfig = jsonController.specifications[kController_CLIENT][kController_CONTROL_MODE];
    [JsonControllerHelper renderControlWithMode: controllerMode jsonTopView:jsonview config:controllModeConfig];
}
...
+(NSMutableDictionary*) getRenderModel: (NSString*)orderType
{
    ...
    NSArray* keys = [MODEL.modelsStructure getModelProperties: orderType];
