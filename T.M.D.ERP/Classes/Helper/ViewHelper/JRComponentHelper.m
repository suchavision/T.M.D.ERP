    ...
    for (NSString* key in config) {
        ...
        [self setupPhotoPickerWithInteractivView: interactiveView handler:^void(AppImagePickerController* imagePickerController) {
            ...
            if (specfication[@"MASK"]) {
                imagePickerController.willShowViewController = ^void(AppImagePickerController* controller, BOOL animated) {
                    ...
                    [maskImageView setSize: [RectHelper getScreenSizeByDeviceOrientation:(UIDeviceOrientation)controller.interfaceOrientation]];
                    [controller.view addSubview: maskImageView];
                    
                    controller.didRotateFromOrientation = ^(UIImagePickerController* controller, UIInterfaceOrientation orientation) {
                        [maskImageView setSize: [RectHelper getScreenSizeByDeviceOrientation:(UIDeviceOrientation)controller.interfaceOrientation]];
                    };
                };
            }
            if (specfication[@"SHOULD_AUTO_ROTATE"]) {
                imagePickerController.shouldAutorotateAction = ^BOOL(AppImagePickerController* controller) {
                    return [specfication[@"SHOULD_AUTO_ROTATE"] boolValue];
                };
            }
...
+(void) showImagePickerWithHandler:(void(^)(AppImagePickerController* imagePickerController))handler
{
    ...
    [PopupTableHelper popTableView:nil keys:@[@"takePhoto", @"photoLibrary"] selectedAction:^(JRButtonsHeaderTableView *sender, NSUInteger selectedIndex, NSString *selectedVisualValue) {
        
        NSString* clickButtonTitle = selectedVisualValue;
        ...
        [VIEW.navigator presentViewController:imagePickerController animated:YES completion: NULL];
    ...
    UIActionSheet* actionSheet = [PopupViewHelper popSheet: nil inView:[ViewHelper getTopView] actionBlock:^(UIView *view, NSInteger buttonIndex){
        ...
        NSString* clickButtonTitle = [sheet buttonTitleAtIndex: buttonIndex];
    ...
    for (NSString* attribute in config) {
        ...
        if ([view isKindOfClass:[JRImageView class]]) {
            ...
            jrimageView.didClickAction = ^void(JRImageView* imageView) {
                ...
                if (jsonController.controlMode == JsonControllerModeNull) {
...
+(void) showDatePicker: (UIView*)obj
{
    ...
    [self showDatePickerView:pickerMode title:nil date:date confirmAction:^(NSDate *selectedDate) {
...
{
    UIView* superView = [[UIView alloc] initWithFrame:CanvasRect(0, 0, 1024, 500)];
    
    // title label
    JRLabel* titleLabel = [[JRLabel alloc] initWithFrame:CanvasRect(0, 0, 200, 50)];
    titleLabel.text = title;
    [titleLabel setCenterX: [superView sizeWidth] / 2];
    [superView addSubview: titleLabel];
    
    
    // Date Picker
    
    UIDatePicker *datePicker = [[UIDatePicker alloc] initWithFrame: CanvasRect(0, 0, 1024, 450)];
    datePicker.datePickerMode = datePickerMode;
    [datePicker setDate:date animated:NO];
    
    [superView addSubview: datePicker];
    datePicker.center = [superView middlePoint];
    [datePicker addOriginY: CanvasY(50)];
    
    [ColorHelper setBackGround:superView color:[UIColor whiteColor]];
    
    // cancel and confirm buttons
    JRButton* cancelButton = [[JRButton alloc] init];
    [cancelButton setBackgroundImage:[UIImage imageNamed:@"Pushbutton_08.png"] forState:UIControlStateNormal];
    cancelButton.frame = CanvasRect(0, 0, 150, 100);
    ...
    JRButton* confirmButton = [[JRButton alloc] init];
    [confirmButton setBackgroundImage:[UIImage imageNamed:@"Pushbutton_08.png"] forState:UIControlStateNormal];
    confirmButton.frame = CanvasRect(874, 0, 150, 100);
    [confirmButton setTitle:LOCALIZE_KEY(@"OK") forState:UIControlStateNormal];
    ...
    confirmButton.didClikcButtonAction = ^void(JRButton* button){
        if (confirmAction) {
    ...
    cancelButton.didClikcButtonAction = ^void(JRButton* button){
        if (cancelAction) {
            cancelAction(datePicker.date);
        }
        [PopupViewHelper dissmissCurrentPopView];
    };
    
    // POP UP
    [PopupViewHelper popView:superView inView:[ViewHelper getTopView] tapOverlayAction:^void(UIControl* control){} willDissmiss: nil];
}
...
+(void) translateNumberToName:(NSMutableDictionary*)objects attributes:(NSArray*)attributes
{
    for (NSString* key in attributes) {
        ...
        if (! OBJECT_EMPYT(number)) {
            NSString* name = MODEL.usersNONames[number];
...
+(NSString*) getNameByNumber: (NSString*)type number:(NSString*)number
{
    ...
    if (!type || [type isEqualToString: MODEL_EMPLOYEE]) {
        name = MODEL.usersNONames[number];
