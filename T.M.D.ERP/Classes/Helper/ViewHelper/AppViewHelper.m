...
{
    return [self getDepartmentsWheelController: NULL];
}

+(AppWheelViewController*) getDepartmentsWheelController: (Class)wheelClazz
{
    AppWheelViewController* departmentWheelController = nil;
    if (wheelClazz != NULL) {
        departmentWheelController = [[wheelClazz alloc] init];
    } else {
        departmentWheelController = [[AppWheelViewController alloc] init];
    }
    NSDictionary* departmentConfig = [DictionaryHelper combines:MODEL.config[@"WHEELS"][@"Common"] with:MODEL.config[@"WHEELS"][@"Categories"]];
    departmentWheelController.config = departmentConfig;
    return departmentWheelController;
    
}

+(AppWheelViewController*) getOrdersWheelController: (NSString*)department
{
    AppWheelViewController* orderWheelController = [[AppWheelViewController alloc] init];
    NSDictionary* orderConfig = [DictionaryHelper combines:MODEL.config[@"WHEELS"][@"Common"] with:MODEL.config[@"WHEELS"][department]];
    orderWheelController.config = orderConfig;
    return orderWheelController;
}
