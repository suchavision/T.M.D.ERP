...
+(void) refreshServerBasicData: (void(^)(BOOL isSuccess))completeHandler
{
    ...
    [MODEL.requester startPostRequest:model completeHandler:^(HTTPRequester* requester, ResponseJsonModel *response, NSHTTPURLResponse *httpURLReqponse, NSError *error) {
...
+(void) dealWithSignedBasicData: (NSArray*)objects
{
    ...
    @synchronized(MODEL) {
        MODEL.usersNONames = usersNONames;
        MODEL.usersNOLevels = usersNOLevels;
        MODEL.usersNOApproval = usersNOApproval;
        MODEL.usersNOResign = usersNOResign;
        MODEL.clientNONames = clientNONames;
        MODEL.approvalSettings = settings;
    }
}
...
{
    NSMutableArray* wheels = [ArrayHelper deepCopy: [MODEL.modelsStructure getAllCategories]];
...
{
    NSArray* temp = [[MODEL.usersNOPermissions objectForKey:  username] objectForKey: CATEGORIES];
...
{
    NSArray* sortedCategoriesWheels = MODEL.config[@"WHEELS"][@"SORTED_Categories"];
...
{
    NSDictionary* userOrdersPermissions = [[[MODEL.usersNOPermissions objectForKey: username] objectForKey: PERMISSIONS] objectForKey: department];
    ...
    // filter , in debug mode, remove it in production
    NSArray* allOrdersInDepartment = [MODEL.modelsStructure getOrders: department withBill:NO];
    ...
    // remove the contains
    [MODEL.modelsStructure removeInsertModelsIn: userOrdersWheels];
    
    NSArray* sortedOrdersWheels = MODEL.config[@"WHEELS"][@"SORTED_Models"][department];
...
+ (NSMutableDictionary*) getApnsContents: (NSString*)department order:(NSString*)order identities:(NSDictionary*)identities forwardUser:(NSString*)forwardUser alert:(NSString*)alert 
{
    ...
    [informations setObject: MODEL.signedUserName forKey:APNS_INFOS_USER_FROM];
