...
{
    return [self check:MODEL.signedUserName department:deparment order:order permission:permission];
}

+(BOOL) check:(NSString*)username department:(NSString*)deparment order:(NSString*)order permission:(NSString*)permission
{
    NSArray* permissions = [[[[MODEL.usersNOPermissions objectForKey: username] objectForKey: PERMISSIONS] objectForKey: deparment] objectForKey:order];
