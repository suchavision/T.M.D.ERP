...

{
    ...
    if (self) {
        ...
        self.downloadURL = [self.urlRequest URL];
        self.delegate = delegateOrNil;
        self.pathToDownloadDirectory = pathToDL;
    }
    return self;
...
- (id)initWithRequest:(NSMutableURLRequest *)request
    ...
    firstResponse:(FirstResponseBlock)firstResponseBlock
         ...
         complete:(CompleteBlock)completeBlock
{
    self = [self initWithRequest:request downloadPath:pathToDL delegate:nil];
    if (self) {
        self.firstResponseBlock = firstResponseBlock;
        self.progressBlock = progressBlock;
        self.errorBlock = errorBlock;
...
- (void)start
{
    NSMutableURLRequest *fileRequest = nil;
    if (self.urlRequest) {
        fileRequest = self.urlRequest;
    } else {
        fileRequest = [NSMutableURLRequest requestWithURL:self.downloadURL
