...
-(NSString*) getCategory: (NSString*)orderType
{
    ...
    for (NSString* departement in allOrders) {
        ...
        for (NSString* order in orders)  if ([order isEqualToString: orderType]) {
            if (category) {
                [AppViewHelper alertError: [NSString stringWithFormat:@"Duplicated Order %@ in %@ and %@", orderType, category, departement]];
            }
...
{
    NSDictionary* visuals = [MODEL.modelsStructure getInsertModelsDefine];
