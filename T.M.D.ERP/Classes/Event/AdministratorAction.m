...
-(void) savePermissions: (NSString*)username permissions:(NSString*)permission categories:(NSString*)categories completeHandler:(void (^)(NSError* errorObj))completeHandler {
    ...
    [MODEL.requester startPostRequest:requestJsonModel completeHandler:^(HTTPRequester* requester, ResponseJsonModel *data, NSHTTPURLResponse *httpURLReqponse, NSError *error) {
...
+(void) initializeAdministrator
{
    ...
    [PopupViewHelper popAlert:APPLOCALIZE(HR_INITAILIZED_ADMIN_KEY) message:LOCALIZE_MESSAGE(@"Ask_Setting_Now") style:0 actionBlock:nil dismissBlock:^(UIView *popView, NSInteger index) {
        if (index == 1) {
            ...
            nextBTN.didClikcButtonAction = ^void(JRButton* button) {
                ...
                dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
                    ...
                    submitBTN.didClikcButtonAction = ^void(JRButton* button) {
                        ...
                        [VIEW.progress show];
                        [MODEL.requester startPostRequest:requestModel completeHandler:^(HTTPRequester* requester, ResponseJsonModel *data, NSHTTPURLResponse *httpURLReqponse, NSError *error) {
                            [VIEW.progress hide];
                            ...
                            if (data.status) {
                                ...
                                [AppViewHelper alertError: @"Create Administrators Failed"];
                            }
                        }];
                    };
