...
- (id <SDWebImageOperation>)downloadImageWithURL:(NSURL *)url options:(SDWebImageDownloaderOptions)options progress:(void (^)(NSInteger, NSInteger))progressBlock completed:(void (^)(UIImage *, NSData *, NSError *, BOOL))completedBlock {
    ...
    [self addProgressCallback:progressBlock andCompletedBlock:completedBlock forURL:url createCallback:^{
        ...
        // isaacs
        NSArray* paths = [[url absoluteString] componentsSeparatedByString: @"?"];
        if (paths.count > 1) {
            NSString* pathString = [NSString stringWithFormat: @"%@=%@", @"PATH", paths[1]];
            NSMutableData* requestData = [NSMutableData dataWithData: [pathString dataUsingEncoding: NSUTF8StringEncoding]];
