            ...
            id <SDWebImageOperation> subOperation = [self.imageDownloader downloadImageWithURL:url options:downloaderOptions progress:progressBlock completed:^(UIImage *downloadedImage, NSData *data, NSError *error, BOOL finished) {
                if (weakOperation.isCancelled) {
                    ...
                    if (options & SDWebImageRefreshCached && image && !downloadedImage) {
                        ...
                        // I modified the order of the call, make it before the callback, because
                        // the the completeblock call back which running on main thread blocked,
                        // but we make a busy watting at main thread, this lead to some dead lock.
