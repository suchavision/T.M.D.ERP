...
@interface MWPhotoBrowser()<YIPopupTextViewDelegate>
...
- (void)performLayout {
    ...
    UIBarButtonItem* remarkButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCompose target:self action:@selector(popremark)];
    [remarkButton setWidth:5];
    [items addObject:remarkButton];
    // Middle - Nav
...
//    NSString* path = [photo.photoURL absoluteString];
//    NSData* imageRawData = [[SDImageCache sharedImageCache] diskImageDataBySearchingAllPathsForKey:path];
//    NSDictionary* meta = [exifTagger getMetaDataFromImage:imageRawData];

    YIPopupTextView* remarkPop = [[YIPopupTextView alloc] initWithPlaceHolder:@"添加备注" maxCount:50];
    remarkPop.text = photo.caption;
    remarkPop.delegate = self;
    [remarkPop showInView:self.view];
}
...
- (void)popupTextView:(YIPopupTextView*)textView willDismissWithText:(NSString*)text{
    ...
    NSString* valid_str = [text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    
    NSUInteger index = [self currentIndex];
    MWPhoto* photo = [self photoAtIndex:index];
    if ([valid_str isEqualToString:photo.caption]) return;

    NSString* path = [photo.photoURL absoluteString];
    NSData* imageRawData = [[SDImageCache sharedImageCache] diskImageDataBySearchingAllPathsForKey:path];
    NSData* newData = [exifTagger tagUserComment:valid_str ToImageData:imageRawData];
    NSString* p = photo.photoURL.absoluteString;
    NSArray* pcoms = [p componentsSeparatedByString:@"?"];
    NSString* vp = [[(NSString*)pcoms[1] stringByDeletingPathExtension] stringByRemovingPercentEncoding];
    MBProgressHUD* hud = [[MBProgressHUD alloc] initWithFrame:CGRectMake(0, 0, 100, 100)];
    hud.labelText = @"正在备注请稍后...";
    [self.view addSubview:hud];
    [hud show:YES];
    [[Store sharedStore] addPhotoAtPath:vp withData:newData complete:^(NSError *error, UIImage *image) {
        [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
        if (!error){
            photo.caption = valid_str;
            [self performLayout];
        }else{
            UIAlertView* faileAlert = [[UIAlertView alloc] initWithTitle:nil message:@"备注失败, 请确保网络通常" delegate:self cancelButtonTitle:@"ok" otherButtonTitles:nil];
            [faileAlert show];
        }
    }];
}
...
//            page.captionView = [[MWCaptionView alloc] initWithPhoto:photo];
